...
void loop()
{
  ...
  if (key != NO_KEY){
    ...
    if (key == '#' || key == '*'){
      ...
      else if ((key=='*' && voiceOrSong == VOICE) || (key=='#' && voiceOrSong == SONG)) {
      ...
      if ((keyspressed < 3) || (digits[1] < 2 && (digits[0] < 2 || (digits[0] == 2 && charToInt(key)<8 ))&& keyspressed < 4)){
        digits[2] = digits [1];
        digits[1] = digits[0];
