...
int channelSelectorButtons[] = { redSelectorButton, yelSelectorButton,
		grnSelectorButton, bluSelectorButton, whtSelectorButton };
...
int channelSelectorLEDs[] = { redSelectorLED, yelSelectorLED, grnSelectorLED,
		bluSelectorLED, whtSelectorLED };
...
int arpeggioLEDs[] = { arpeggioRedLED, arpeggioGrnLED, arpeggioBluLED };
...
int glissLEDs[] = { glissRedLED, glissGrnLED, glissBluLED };
...
void updateChannelProps(int button, int* lastState, int* rgbPins,
		boolean* moduleState, byte data1) {
...
void loop() {
	...
	if (isPressed(redSelectorButton) && isPressed(bluSelectorButton)
			&& isPressed(yelSelectorButton) && isPressed(grnSelectorButton)
			&& isPressed(whtSelectorButton)) {
		mushroom();
		demoSequence();
	}
	
	// Argeggio and glissando have similar logic
	updateChannelProps(arpeggioButton, &lastArpeggioState, arpeggioLEDs,
			arpeggioOn, ARPEGTOGGLE);
